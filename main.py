...
def main():
    ...
    logger = setup_logger(output=args.output, distributed_rank=dist.get_rank(), color=False, name="MARL")
...
def main_worker(args, logger):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.val_interval == 0:
            ...
            loss_ema, mAP_ema = validate(val_loader, ema_m.module, criterion, args, logger, epoch)
            losses.update(loss)
            mAPs.update(mAP)
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                if args.loss_dev > 0:
                    loss *= args.loss_dev
                # output_sm = nn.functional.sigmoid(output)    #raise Userwarning   Use torch.sigmoid instead
